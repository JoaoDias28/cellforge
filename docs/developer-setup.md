...
| PyYAML 6 (`python3-yaml` in Ubuntu/ROS) | MIT | Actively maintained and distributed by Ubuntu | Parse frozen YAML recipe documents inside the offline job gateway; safe loading is used and recipes remain data-only | Materialize canonical JSON recipes in deployment bundles and replace the gateway loader before removing the package |
