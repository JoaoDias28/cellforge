...
| nlohmann/json 3 (`nlohmann_json`, supplied by Jazzy's `nlohmann_json_schema_validator_vendor`) | MIT | Actively maintained upstream and packaged by Ubuntu/ROS | Parse the immutable bundle and behavior-tree node manifests before loading any runtime plugin | Replace the supervisor manifest reader with another supported JSON parser while preserving exact digest, containment, declaration, and registration validation |
